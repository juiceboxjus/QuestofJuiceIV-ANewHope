...
▀█████▀ ▀███▀ ██▄▄▄ ▄▄██▀   ██     ▀███▀ ██      ████▀ ▀███▀ ██ ▀████ ██▄▄▄ ██▄▄▄ ██▀██ ██ ▀██ ████▀ ██ ██▀██
     ▀▀                                                                                                      
 """)
scroll_print("Hello, Brave adventurer! Welcome to the Quest of Juice IV: A New Hope.")
...
# build the combat function

 

def combat(player, enemy):
    scroll_print(f"*Combat Theme* Battle Time! The {enemy['name']} approaches!!")
    base_defense = player['defense']  # store original defense
    while player['health'] > 0 and enemy['health'] > 0:
        ...
        if action == "attack":
            damage = max(0, player["attack"] - enemy["defense"] + random.randint(-5, 6))
            enemy['health'] -= damage
            scroll_print(f"You deal {damage} damage!")

        elif action == "defend":
            scroll_print("You raise your shield in anticipation of an attack...")
            player['defense'] += 2

        else:

            scroll_print("Invalid action! You lose your turn!")

        # ENEMY TURN (MUST BE INSIDE LOOP)
        ...
        # this should reset player defense after each turn
